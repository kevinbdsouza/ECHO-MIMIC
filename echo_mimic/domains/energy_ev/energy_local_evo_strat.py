...
from .prompts import build_stage_two_prompt
from .evaluation import evaluate_local_agent_policy_script
...
LOCAL_GUIDELINES = (
    ...
    "- Emit a 7-element list of per-slot usage vectors (one per day, four floats in [0, 1]) to local_policy_output.json via json.dump.\n"
...
def init_models(model_name: str) -> tuple:
    ...
    system_instructions = (
        ...
        "Produce concise, fully runnable policy.py files that reason over the scenario context and output a 7-day per-slot usage plan with values between 0 and 1."
    )
    fix_instructions = (
        "You repair Python scripts for EV charging heuristics. Given failing code and stderr, return a corrected policy.py that writes seven per-slot usage vectors."
    )
