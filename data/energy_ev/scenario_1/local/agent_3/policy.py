...
NEIGHBOR_EXAMPLES = [
    # Neighbor 2 (Location 2, Transformer headroom focus) -> Chosen as most behaviorally aligned (prefers slots 1, 2)
...
class Agent3Policy:
    ...
    def _get_imitation_targets(self):
        """
        Agent 3 (Location 3, Peak demand slot 2) aligns best behaviorally with Neighbor 2 
        (Prefers slots 1, 2) over Neighbor 5 (Prefers slots 0, 1). We imitate Neighbor 2's historical plan.
        """
        return NEIGHBOR_EXAMPLES[0]["ground_truth"]


    def calculate_cost(self, day_data, slot_idx):
        """Calculate the composite cost (Not strictly used for Stage 2 output, but kept for structure)."""
        price_cost = day_data['Tariff'][slot_idx]
        agent_spatial_carbon = day_data['Spatial carbon'][self.location][slot_idx]
