...
def calculate_load_metric(day_data, base_demand):
    """
    Metric prioritizes lowest baseline load for headroom concerns. 
    Penalty applied for slots restricted to 1 session.
    """
    metrics = []
    ...
    for i in range(4):
        ...
        # Penalty if baseline load is very low, suggesting high utilization margin, 
        # but we prioritize absolute lowest baseline load first.
        if baseline_loads[i] < CONTEXT['slot_min_sessions'][i] * 2.0:
             load_proxy += 10.0 

        # Small penalty for slots that are highly capacity-constrained by session limits
...
def select_slot(day_data):
    """Selects the slot with the minimum load metric, breaking ties with minimum tariff."""
    ...
    # Tie-breaking: use lowest tariff among the best (lowest load) slots
    if len(best_slots) > 1:
...
def generate_policy():
    policy = []
    # Process days in order (Day 1 -> Day 7)
