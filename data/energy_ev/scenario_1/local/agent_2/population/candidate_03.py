...
def calculate_load_metric(day_data, base_demand):
    ...
    for i in range(4):
        ...
        # Headroom concern: Penalize if baseline load is barely above minimum session requirement proxy
        if baseline_loads[i] < CONTEXT['slot_min_sessions'][i] * 2.0:
             load_proxy += 10.0 

        # Headroom concern: Penalize slots that are highly constrained (max sessions = 1)
...
def select_slot(day_data):
    ...
    # Imitation stage tie-breaker: If load metrics are identical, choose the one with the lowest tariff
    if len(best_slots) > 1:
