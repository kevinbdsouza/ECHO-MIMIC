...
def calculate_load_metric(day_data, base_demand):
    ...
    for i in range(4):
        ...
        # Heuristic: Penalize slots near min sessions if load is low (as per previous analysis)
        if baseline_loads[i] < CONTEXT['slot_min_sessions'][i] * 2.0:
             load_proxy += 10.0 

        # Penalty for slots that are highly capacity constrained (max session = 1)
...
def select_slot(day_data):
    ...
    # Tie-breaking: Favor lower tariff among minimum load slots
    if len(best_slots) > 1:
